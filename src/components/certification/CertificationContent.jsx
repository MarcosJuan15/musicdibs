...
export default function CertificationContent() {
  const t = useTranslations('certification.content');
  const [zoom, setZoom] = useState(100);

  const certificateData = t.raw('certificate_data');

  const handleZoomIn = () => setZoom(prev => Math.min(prev + 10, 200));
  const handleZoomOut = () => setZoom(prev => Math.max(prev - 10, 50));

  const handleViewBlockchain = () => {
    window.open(t('blockchain_url'), "_blank");
  };

  const handleDownloadPDF = () => {
    const link = document.createElement('a');
    link.href = t('pdf_url');
    link.download = t('pdf_filename');
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-200">
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className="text-sm text-gray-500 font-medium">{t('zoom.auto')}</span>
              </div>
